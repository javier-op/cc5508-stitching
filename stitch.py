...
def stitch(img1, img2):
    stitched = np.maximum(img1, img2)
    min_i = None
    for i in range(stitched.shape[0]):
        if not min_i:
            for j in range(stitched.shape[1]):
                if stitched[i][j][0] > 0 or stitched[i][j][1] > 0 or stitched[i][j][2] > 0:
                    min_i = i
                    break
        else:
            break
...
img1 = cv2.imread('./images/caso_2/2a.jpg', cv2.IMREAD_COLOR)
img2 = cv2.imread('./images/caso_2/2b.jpg', cv2.IMREAD_COLOR)
